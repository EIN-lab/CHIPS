function CompositeImgObj = from_files(varargin)
%from_files - Create a CompositeImg object from a list of files
%
%   OBJ = CompositeImg.from_files() prompts for all required information
%   and creates a CompositeImg object from existing files.
%
%   OBJ = CompositeImg.from_files(RAWIMGTYPE) loads the files into object
...
%
%   OBJ = CompositeImg.from_files(RAWIMGTYPE, CONFIG) uses the specified
...
%
%   OBJ = CompositeImg.from_files(RAWIMGTYPE, CONFIG, PROCTYPE) creates
...
%
%   OBJ = CompositeImg.from_files(RAWIMGTYPE, CONFIG, PROCTYPE, MASK)
...
%
%   OBJ = CompositeImg.from_files(NAME, ...) specifies a name for the
%   CompositeImg object. NAME must be a single row character array, or a
%   cell array containging a single row character array.
%
%   See also CompositeImg.add, RawImg.from_files, RawImgComposite
...
%   along with this program.  If not, see <http://www.gnu.org/licenses/>.
    ...
    % Create the CompositeImg object
    CompositeImgObj = CompositeImg(nameIn, rawImgArray, config, ...
        procType, masks);

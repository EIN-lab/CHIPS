function varargout = plot_ICAsigs(self, objPI, hAx, params)
...
ica_filters = objPI.calcFindROIs.data.icFilters;
ica_sig = objPI.calcFindROIs.data.icTraces;
dt = 1 / objPI.rawImg.metadata.frameRate;
tlims = [objPI.calcFindROIs.data.time(1) objPI.calcFindROIs.data.time(end)];

if isequal(params.plotROIs, objPI.calcMeasureROIs.get_plotROIs([]))
    ICuse = self.config.ICuse;
else
    labs = objPI.calcFindROIs.data.segmentlabel;
    ICuse = unique(labs(params.plotROIs));
end
...
% ---------------------------------------------------------------------%
% These should be obtained via the new CalcDetectSigsCellSort - OK
mode = self.config.mode;
plottype = self.config.plottype;
ratebin = self.config.ratebin * dt;
spt = self.data.spt;
spc = self.data.spc;
% ---------------------------------------------------------------------%
...
utils.checks.greater_than(min(ICuse), 1, true, 'ICuse');
utils.checks.less_than(max(ICuse), nIC, true, 'ICuse');

if isempty(ratebin)
    ratebin = 10*dt;
else
    utils.checks.prfs(ratebin, 'ratebin')
    utils.checks.greater_than(ratebin, dt, true, 'ratebin');
end

%pick picture to overlay icROIs on
if self.config.background == 1
    f0 = objPI.calcFindROIs.data.tAverage;
elseif self.config.background == 2
    f0 = mean(objPI.calcFindROIs.data.icFilters(:,:, ICuse), 3);
end
...
switch mode
                ...
                switch plottype
                    ...
                    case 4
                        ...
                        for jj=1:length(ICuseuse)
                            ...
                            % MH 10/11/2017 
                            % prints IC mean rate for each all ICs
                            % fprintf('%3.3f\n', sprm(jj)) 
                            box off
    ...
    case {'contour'}
        ...
        formataxes
        axis image tight off
        title('Temporal average of sequence, with contours of ICs')

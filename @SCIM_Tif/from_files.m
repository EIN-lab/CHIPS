...
%
%   OBJ = SCIM_Tif.from_files() prompts for all required information and
%   creates a SCIM_Tif object.
%
%   OBJ = SCIM_Tif.from_files(FILENAME) creates an object from the
...
%   files.
%
%   OBJ = SCIM_Tif.from_files(FILENAME, CHS) specifies the metadata
...
%
%   OBJ = SCIM_Tif.from_files(FILENAME, CHS, CAL) specifies the pixel size
%   calibration for the image.  CAL must be a scalar CalibrationPixelSize
%   object.  If calibration is empty, the constructor will prompt to select
%   one.
%
%   See also RawImg.from_files, SCIM_Tif.SCIM_Tif, Metadata,

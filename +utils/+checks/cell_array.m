function varargout = cell_array(val, varargin)
%cell_array - Check that the value is a cell array
%
%   cell_array(val) checks that val is a cell array.  If it is, nothing
%   else happens; if it is not, the function throws an exception from the
%   calling function.
%
%   cell_array(val, 'VarName') includes VarName in the exception message.
%
%   ME = cell_array(...) returns the MException object instead of throwing
...
%
%   See also iscell, error, MException, MException.throwAsCaller
...
%% Check the value is a cell array

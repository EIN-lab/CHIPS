function varargout = less_than(val, num, allowEq, varargin)
%less_than - Check that a value is less than a number
%
%   less_than(val) checks that all elements of val are less than 1.  If
%   they are, nothing else happens; if it is not, the function throws an
%   exception from the calling function.  val must be numeric, real and
%   finite.
%
%   less_than(val, num) checks that all elements of val are less than num.
%   If num is empty or not specified, it is assumed to be 1. num must be a
%   scalar real number.
%
%   less_than(val, num, allowEq) specifies whether to allow cases where val
%   is equal to num.  If allowEq is empty or not specified, it is assumed
%   to be false.  allowEq must be a scalar value that can be converted to a
%   logical.
%
%   less_than(val, num, allowEq, 'VarName') includes VarName in the
%   exception message.
%
%   ME = less_than(...) returns the MException object instead of
...
%
%   See also lt, le, error, MException, MException.throwAsCaller
...
if ~isGood
    ...
    if allowEq
        strComp = 'less than or equal to';
    else
        strComp = 'less than';
    end
    
    % Create the MException object
    ME = MException('Utils:Checks:LessThan', ['All elements of %s ' ...

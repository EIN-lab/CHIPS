function varargout = not_empty(val, varargin)
%not_empty - Check that the value is not empty
%
%   not_empty(val) checks that val is not empty.  If it is, nothing else
%   happens; if it is not, the function throws an exception from the
%   calling function.
%
%   not_empty(val, 'VarName') includes VarName in the exception message.
%
%   ME = not_empty(...) returns the MException object instead of throwing
...
%
%   See also isempty, error, MException, MException.throwAsCaller
...
if ~isGood
    ...
    ME = MException('Utils:Checks:Empty', 'The %s must not be empty', ...
        varName);

function varargout = prfsi(val, varargin)
%prfsi - Check that the value is a positive, real, finite, scalar integer
%
%   prfsi(val) checks that val is a positive, real, finite, scalar integer.
%   If it is, nothing else happens; if it is not, the function throws an
%   exception from the calling function.
%
%   prfsi(val, 'VarName') includes VarName in the exception message.
%
%   ME = prfsi(...) returns the MException object instead of throwing it
...
%   utils.checks.finite, utils.checks.scalar, utils.checks.integer, 

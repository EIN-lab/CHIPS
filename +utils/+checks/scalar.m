function varargout = scalar(val, varargin)
%scalar - Check that the value is scalar
%
%   scalar(val) checks that val is scalar.  If it is, nothing else happens;
%   if it is not, the function throws an exception from the calling
%   function.
%
%   scalar(val, 'VarName') includes VarName in the exception message.
%
%   ME = scalar(...) returns the MException object instead of throwing it
...
%
%   See also isscalar, error, MException, MException.throwAsCaller
...
if ~isGood
    ...
    strSize = sprintf('%d ', size(val));
    ME = MException('Utils:Checks:Scalar', ['The %s must be scalar, ' ...
        'but the data provided was of size [%s].'], ...
        varName, strSize(1:end-1));

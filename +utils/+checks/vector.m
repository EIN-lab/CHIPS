function varargout = vector(val, varargin)
%vector - Check that the value is a vector
%
%   vector(val) checks that val is a vector.  If it is, nothing else
%   happens; if it is not, the function throws an exception from the
%   calling function.
%
%   vector(val, 'VarName') includes VarName in the exception message.
%
%   ME = vector(...) returns the MException object instead of throwing it
...
%
%   See also isvector, error, MException, MException.throwAsCaller
...
if ~isGood
    ...
    ME = MException('Utils:Checks:Vector', ['The %s must be a vector, ' ...

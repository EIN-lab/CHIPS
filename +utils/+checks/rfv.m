function varargout = rfv(val, varargin)
%rfv - Check that the value is a real, finite vector
%
%   rfv(val) checks that val is a real, finite, vector. If it is, nothing
%   else happens; if it is not, the function throws an exception from the
%   calling function.
%
%   rfv(val, 'VarName') includes VarName in the exception message.
%
%   ME = rfv(...) returns the MException object instead of throwing it
...
%
%   See also utils.checks.real_num, utils.checks.finite,
%   utils.checks.vector, MException, MException.throwAsCaller

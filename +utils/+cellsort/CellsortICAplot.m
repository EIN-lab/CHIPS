function CellsortICAplot(mode, ica_filters, ica_sig, f0, tlims,...
    dt, ratebin, plottype, ICuse, spt, spc)
...
if nargin<9 || isempty(ICuse) || length(ICuse) > nIC
    ICuse = [1:nIC];
end
if size(ICuse,2)==1
    ICuse = ICuse';
...
for i = 1:length(ICuse)
    zsig(i, :) = utils.zscore(sig(ICuse(i),:));
end

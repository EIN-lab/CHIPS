...
if n > 1
    ...
    for a = 1:n
		try
			...
			A(:,:,:,a) = utils.sc_pkg.sc(I(:,:,:,a), varargin{:});
		end
    end
    I = A;
    if nargout == 0
		...
		try
			...
			utils.sc_pkg.imdisp(A, 'Size', sz, 'Border', 0.01); 
		end
    end
    return
end
...
if nargout == 0
    ...
    try
        ...
        hIm = utils.sc_pkg.imdisp(I, 'Border', 0);
    end
    ...
    case 'phase'
        ...
        if diff(limits)
            ...
            try
                ...
                I = utils.sc_pkg.real2rgb(mod(I(:,1) / (2 * pi), 1), 'hsv');
            end
        end
        limits = []; % This colormap doesn't have a valid colorbar
%% RGB2Gray
...
%% Prob
    case 'prob'
        ...
        try
            ...
            [I, limits] = utils.sc_pkg.real2rgb(I, cmap2, limits);
        end
...
%% Gray_jet
    case 'gray_jet'
        ...
        try
            ...
            J = utils.sc_pkg.real2rgb(I(:,:,1), 'jet', limits);
        end
        try
            G = real2rgb(I(:,:,2), 'gray', limits);
        catch
            G = utils.sc_pkg.real2rgb(I(:,:,2), 'gray', limits);
        end
        try
            A = real2rgb(I(:,:,3), 'gray', limits);
        catch
            A = utils.sc_pkg.real2rgb(I(:,:,3), 'gray', limits);
        end
        I = J .* A + G .* (1 - A);
        limits = [];
%% Compress
...
%% Contrast
    case 'contrast'
        ...
        try
            ...
            I = utils.sc_pkg.real2rgb(I, map, [0 1]);
        end
%% Stereo (anaglyph)
    case 'stereo'
    ...
    if ischar(mask{1,a})
        if numel(mask{1,a}) == 1
            ...
            try
                ...
                mask{1,a} = utils.sc_pkg.sc(I, mask{1,a});
            end
        end
    end
...
try
    ...
    [B, map, map] = utils.sc_pkg.real2rgb(B, cmap, [0 1]);
end
return
...
figure(fig); clf;
load mandrill; mandrill = ind2rgb(X, map);
try
    sc(mandrill);
catch
    utils.sc_pkg.sc(mandrill);
end
display_text([...
...
%% Demo discretization
figure(fig); clf;
subplot(121); try sc(Z, 'jet'); catch, utils.sc_pkg.sc(Z, 'jet'); end, label(Z, 'sc(Z, ''jet'')');
...
figure(fig); clf;
subplot(221); try sc(Z, 'hsv'); catch, utils.sc_pkg.sc(Z, 'hsv'); end, label(Z, 'sc(Z, ''hsv'')');
subplot(222); try sc(Z, 'colorcube'); catch, utils.sc_pkg.sc(Z, 'colorcube'); end, label(Z, 'sc(Z, ''colorcube'')');
subplot(223); try sc(Z, 'hicontrast'); catch, utils.sc_pkg.sc(Z, 'hicontrast'); end, label(Z, 'sc(Z, ''hicontrast'')');
subplot(224); try sc(Z-round(Z), 'diff'); catch, utils.sc_pkg.sc(Z-round(Z), 'diff'); end, label(Z, 'sc(Z-round(Z), ''diff'')');
display_text([...
...
figure(fig); clf;
subplot(121); try sc(Z, [0 max(Z(:))], '-hot'); catch, utils.sc_pkg.sc(Z, [0 max(Z(:))], '-hot'); end, label(Z, 'sc(Z, [0 max(Z(:))], ''-hot'')');
subplot(122); try sc(mandrill, [-0.5 0.5]); catch, utils.sc_pkg.sc(mandrill, [-0.5 0.5]); end, label(mandrill, 'sc(mandrill, [-0.5 0.5])');
display_text([...
...
load gatlin;
gatlin = X;
figure(fig); clf; im = cat(3, abs(Z)', gatlin(1:256,end-255:end)); try sc(im, 'prob'); catch, utils.sc_pkg.sc(im, 'prob'); end
...
try 
    ...
    utils.sc_pkg.sc(Z, [0 max(Z(:))], '-hot', utils.sc_pkg.sc(Z-round(Z), 'diff'), Z < 0);
end
display_text([...
...
try
    ...
    utils.sc_pkg.sc(abs(Z), rand(10, 3));
end
colorbar;
display_text([...
...
try
    ...
    utils.sc_pkg.sc(abs(Z), [rand(10, 3) exp((1:10)/2)']);
end
colorbar;
display_text([...
...
try
    ...
    utils.sc_pkg.sc(squeeze(mri(:,:,:,1:6)), 'compress');
end
display_text([...
...
try
    ...
    surf(Z, utils.sc_pkg.sc(Z, 'hicontrast'), 'edgecolor', 'none');
end
display_text([...

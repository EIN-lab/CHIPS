...
elseif n == 1
    ...
    if n > num
        ...
        state.n = n;
        state.I = I;
        strTitle = sprintf(['Images %d to %d [L->R, T->B]. ', ...
            'Use arrow keys to move.'], index(1), index(end));
        % Set the callback for image navigation, and save the image data in the figure
        set(hFig, 'KeyPressFcn', @keypress_callback, 'Interruptible', 'off', 'BusyAction', 'cancel', 'UserData', state, 'Name', strTitle);
    end
...
state.index = index(1);
strTitle = sprintf('Images %d to %d [L->R, T->B]. Use arrow keys to move.', ...
    index(1), min([size(state.I, 4), index(end-1)]));
set(fig, 'UserData', state, 'Name', strTitle);
return
        ...
        case 'map'
            ...
            if ischar(map)
                try
                    ...
                    map = feval(['utils.sc_pkg.' map], 256);
                end
            end
        case {'size', 'grid'}
            layout = inputs{a+1};

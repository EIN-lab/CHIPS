...
%   along with this program.  If not, see <http://www.gnu.org/licenses/>.
    ...
    % Calculate the absolute velocity from the streak angle, 
    % pixel size, and line time
    velocity = (pixelSize / lineTime) .* ...
        (self.data.pixelShift / self.config.shiftAmt);
    ...
    % Mask points with excessively high or low velocity 
    % (determined by the number of standard deviations away from 
    % the median), ignoring those points with low SNR in these 
    % calcultions.
    ...
    self.data = self.data.add_processed_data(velocity);

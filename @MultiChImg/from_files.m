function MultiChImgObj = from_files(varargin)
%from_files - Create a MultiChImg object from a list of files
%
%   OBJ = MultiChImg.from_files() prompts for all required information
%   and creates a MultiChImg object from existing files.
%
%   OBJ = MultiChImg.from_files(RAWIMGTYPE) loads the files into object
...
%
%   OBJ = MultiChImg.from_files(RAWIMGTYPE, CONFIG) uses the specified
...
%
%   OBJ = MultiChImg.from_files(RAWIMGTYPE, CONFIG, PROCTYPE) creates
...
%
%   OBJ = MultiChImg.from_files(RAWIMGTYPE, CONFIG, PROCTYPE, MASK)
...
%
%   OBJ = MultiChImg.from_files(NAME, ...) specifies a name for the
%   MultiChImg object. NAME must be a single row character array, or a
%   cell array containging a single row character array.
%
%   See also MultiChImg.add, RawImg.from_files, RawImgComposite
...
%   along with this program.  If not, see <http://www.gnu.org/licenses/>.
    ...
    % Create the MultiChImg object
    MultiChImgObj = MultiChImg(nameIn, rawImgArray, config, ...
        procType, masks);
